...
class pina_vit(nn.Module):
    def __init__(self, args):
        super(pina_vit, self).__init__()
        ...
        ##### classifier ########################################
        self.unified_classifier = nn.Linear(args['image_dim'], self.class_num, bias=True)
    ...
    def forward(self, image):
        ...
        if self.ca_mode == 'shallow':
            image_features = self.image_encoder(image, img_prompt = img_prompt)
            if self.numtask > 1:
                up = self.up_pool[self.numtask-1]
                down = self.down_pool[self.numtask-1]
                image_features = image_features + up(down(image_features))
        ...
        ##### classifier ############################################################
        logits.append(self.unified_classifier(image_features))
    ...
    def interface(self, image, selection):
        ...
        ##### backbone ############################################################
        if self.ca_mode == 'shallow':
            image_features = self.image_encoder(image, img_prompt = instance_batch)
            _bias_list = []
            for i in range(self.total_sessions):
                _bias = self.up_pool[i](self.down_pool[i](image_features))
                _bias_list.append(_bias)
            # [6, bs, 768] -> [bs, 768]
            selected_bias = torch.stack(_bias_list, 0)[selection, torch.arange(image_features.shape[0])]
            image_features = image_features + selected_bias
        ...
        ##### classifier ############################################################
        logits = self.unified_classifier(image_features)
        return {
            'logits': logits,
        }
